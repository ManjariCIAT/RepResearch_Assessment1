...
#### remove NAs
data_act.RemovedNAs <- data_act[!is.na(data_un$steps), ]
...
qplot(StepsTotal$Steps, geom="histogram", ylab="Number of Days", xlab="Number of Steps taken each day", color = "blue", binwidth = 100, main = "Daily Steps")
```
...
```{r, echo=TRUE}
meanStepInterval <- tapply(data_act.RemovedNAs$steps, as.factor(data_act.RemovedNAs$interval), mean)
...
plot(meanStepsInterval$interval, meanStepsInterval$mean, type = 'l', ylab = "Mean Steps", xlab = "Interval", main = "Average Number of Steps Taken")
```
